function W_lda = MvDAvc1(X_multiview,Label_multiview,lambda,D,C_multiview)
num_class = length(unique(Label_multiview{1}));
num_view = size(X_multiview,2);   % original 2
...
for vi = 1:num_view
    ...
    [Ci ci li] = GetEachClass(C_multiview{vi},Label_multiview{vi},'x'); %%%%%%%%%%
...
for i=1:num_view
    ...
    for j=i:num_view
        ...
        for ci = 1:num_class
            ...
            %vij = vij + Xj{ci} * (Xj{ci}');
            vij = vij + (Xj{ci} * (Xj{ci}')+0.001*((Xj{ci}-Dj*Cj{ci})*(Xj{ci}-Dj*Cj{ci})'));  %0.001 for most; 0.0001 for basketball and walking
        end
        if j==i
            sij = vij + sij;
        end
...
for i=1:num_view
    ...
    %W_lda{i} = (W_LDA(sum(dim(1:i-1))+1:sum(dim(1:i)), sum(dim(1:i-1))+1:sum(dim(1:i))));
end
