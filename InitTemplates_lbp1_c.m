function [T,T_norm,T_mean,T_std] = InitTemplates_lbp1_c(tsize, img_name, cpt)
...
p{21} = cpt + [0 0 0; 0 6 0];
p{22} = cpt + [0 0 0; 0 0 -6];
p{23} = cpt + [0 0 0; 0 0 6];
p{24} = cpt + [0 0 0; 0 0 -6];
p{25} = cpt + [-7 0 0; 0 0 0];
p{26} = cpt + [7 0 0; 0 0 0];
p{27} = cpt + [0 -7 0; 0 0 0];
p{28} = cpt + [0 7 0; 0 0 0];
p{29} = cpt + [0 0 -7; 0 0 0];
p{30} = cpt + [0 0 7; 0 0 0];
p{31} = cpt + [0 0 0; -7 0 0];
p{32} = cpt + [0 0 0; 7 0 0];
p{33} = cpt + [0 0 0; 0 -7 0];
p{34} = cpt + [0 0 0; 0 7 0];
p{35} = cpt + [0 0 0; 0 0 -7];
p{36} = cpt + [0 0 0; 0 0 7];
p{37} = cpt + [-8 0 0; 0 0 0];
p{38} = cpt + [8 0 0; 0 0 0];
p{39} = cpt + [0 -8 0; 0 0 0];
p{40} = cpt + [0 8 0; 0 0 0];
...
T	= zeros(256,10); % 196 for size 16*16; 900 for size 32*32
T_norm = zeros(1,10);
T_mean = zeros(1,10);
T_std = zeros(1,10);
